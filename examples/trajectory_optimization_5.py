...
# Modified based on the trajectory_optimization_3.py
# Revise the search method: search in a plane rather than a sphere.
# Try max the xy distance between each key point.
...
if __name__ == '__main__':
    ...
    for n in range(0, 1):
        ...
        # Step 1: 使用随机搜索来优化粒子xy平面的距离，以及粒子运动学
        min_xy_dists = np.zeros((np.max(max_length_int), 1))
        step_counter = int(0)
        #while np.any(min_xy_dists <= 0.014):
        for m in range(20):
            # Calculate the distance between particles in the xy plane
            ...
            # print初始distances
            if step_counter == 0:
                print('Min distance before random search: \n', min_xy_dists)
            min_xy_dists[0] = 1.0
            min_xy_dists[-1] = 1.0
            min_dist_index = np.argmin(min_xy_dists)
            # print(min_dist_index)
            # print(min_xy_dists[min_dist_index])
            step_counter += 1


            # 对最小xy距离的state进行随机搜索以增大距离
            # Generate 100 potential solutions for the worst state, and then sort them based on the xy distance
            # 100个solution和10个solution是否有很大区别？
            solutions = np.transpose(create_constrained_points_2(
                n_particles, 
                split_data_numpy[:, min_dist_index, 2:], 
                split_data_numpy[:, min_dist_index-1, 2:], 
                split_data_numpy[:, min_dist_index+1, 2:]), 
                (1, 0, 2))
