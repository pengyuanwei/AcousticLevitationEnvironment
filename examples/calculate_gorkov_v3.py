...
def calculate_gorkov_twgs(split_data_numpy, n_particles, transducer, delta, y0, num_transducer, k1, k2, K, T_in, T_out):
    ...
    for i in range(split_data_numpy.shape[1]):
        ...
        A = utilities.piston_model_new(transducer, points1)
        A = A.to(torch.complex64)

        if i == 0:
            hologram_x, field_y = temp_wgs.wgs(A, y0, K)
        else:
            hologram_x, field_y = temp_wgs.temporal_wgs(A, field_y, K, hologram_x, field_y, T_in, T_out)
        
        Ax2, Ay2, Az2 = utilities.surround_points(transducer, points1, delta)
        Ax2 = Ax2.to(torch.complex64)
        Ay2 = Ay2.to(torch.complex64)
        Az2 = Az2.to(torch.complex64)
        ph = torch.angle(hologram_x) + torch.cat((torch.zeros(int(num_transducer/2),1),math.pi*torch.ones(int(num_transducer/2),1)),axis=0)
        gorkov, _ , _ , _ , _  = utilities.forward_full_gorkov(ph, A, Ax2, Ay2, Az2, k1, k2)
...
if __name__ == '__main__':
    ...
    for n in range(10):
        ...
        gorkov, _ = calculate_gorkov_twgs(split_data_numpy, n_particles, transducer, delta, y0, num_transducer, k1, k2, K, T_in, T_out)
    ...
    np.save('gorkovDistribution_'+model+'_twgs.npy', gorkov_concatenated)
