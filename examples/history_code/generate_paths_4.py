...
if __name__ == "__main__":
    ...
    for n in range(30):  
        ...
        original_paths, smoothed_paths, failure = generate_paths_smoothing(global_env, agents[0], n_particles, max_timesteps, levitator)            

        if failure:
            print('1st planner failure!')
            original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(replan_env, agents[1], n_particles, max_timesteps, original_paths, levitator)

            if failure:
                print('2nd planner failure!')
                original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(replan_env, agents[2], n_particles, max_timesteps, original_paths, levitator)

                if failure:
                    print('3rd planner failure!')
                    original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(replan_env, agents[3], n_particles, max_timesteps, original_paths, levitator)

                    if failure:
                        print('4th planner failure!')
                        original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(nonAcoustic_replan_env, agents[0], n_particles, max_timesteps, original_paths)
                        show(n_particles, np.transpose(paths, (1, 0, 2)))

                        if failure:
                            print('5th planner failure!')
                            original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(nonAcoustic_replan_env, agents[1], n_particles, max_timesteps, original_paths)

                            if failure:
                                print('6th planner failure!')
                                original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(nonAcoustic_replan_env, agents[2], n_particles, max_timesteps, original_paths)

                                if failure:
                                    print('7th planner failure!')
                                    original_paths, smoothed_paths, failure = generate_replan_paths_smoothing(nonAcoustic_replan_env, agents[3], n_particles, max_timesteps, original_paths)
