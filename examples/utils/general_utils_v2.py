...
def read_paths(csv_data):
    ...
    Output:
         ...
         - bool indicating if NaN is present)
    """
    try:
        # 转换为浮点数组，并过滤长度不是5的行
        csv_data_float = [
...
def find_nearest_nonzero(arr, index):
    '''
    遍历数组，向前和向后查找最近的非零元素
    '''
    n = len(arr)
...
def lerp_3d_series(p1, p2, t_values):
    # 归一化 t_values，使其范围映射到 [0, 1]
    t_min, t_max = t_values[0], t_values[-1]
    t_norm = (t_values - t_min) / (t_max - t_min)

    # 计算插值点
    points = (1 - t_norm[:, np.newaxis]) * np.array(p1) + t_norm[:, np.newaxis] * np.array(p2)
    return points