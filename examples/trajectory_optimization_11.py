...
# Modified based on the trajectory_optimization_10.py: 优化插值后的Gorkov


if __name__ == '__main__':
    n_particles = 8
    global_model_dir_1 = './experiments/experiment_20'
    model_name = '20_19_98_99/planner_v10'
    num_file = 30
    file_name = 'smoothed_path'
    ...
    for n in range(80):
        ...
        for m in range(10):
            # Calculate Gorkov
            max_gorkovs = np.zeros((paths_length, n_particles))
            for i in range(1, paths_length-1):
                positions = split_data[:, i+1, 2:]
                last_positions = split_data[:, i, 2:]
                # 在两个keypoints之间进行插值
                # paths: (num_particles, path_lengths, 3)
                _, _, _, paths, _ = uniform_velocity_interpolation_v2(
                    start=last_positions, end=positions, total_time=delta_time, dt=0.0032, velocities=0.0
                )
                # 计算插值后的所有轨迹的所有坐标中的最大gorkov
                gorkov = levitator.calculate_gorkov(paths)
                max_gorkovs[i] = np.max(gorkov, axis=0)
            max_gorkov_idx, max_gorkov, sorted_indices = calculate_max_gorkov_v2(max_gorkovs)
            ...
            if len(worst_idx) >= 2 and worst_idx[-1] == worst_idx[-2] == max_gorkov_idx:
                print("Optimizing the next worst Gorkov!")
                max_gorkov_idx = sorted_indices[-2]
            worst_idx.append(max_gorkov_idx)
            
            candidate_solutions, sorted_indices, sorted_solutions_max_gorkov = generate_solutions_whole_paths_v2(
                n_particles, split_data, max_gorkov_idx, levitator, delta_time
            )
        ...
        for i in range(1, paths_length-1):
            ...
            gorkov = levitator.calculate_gorkov(paths)
            max_gorkovs[i] = np.max(gorkov, axis=0)
        max_gorkov = np.max(max_gorkovs, axis=1)
        ...
        for m in range(1, split_data.shape[1] - 1):
            if positions_check(split_data[:, m, 2:], split_data[:, m-1, 2:], split_data[:, m+1, 2:]):
                ...
                candidate_solutions, sorted_indices, sorted_solutions_max_gorkov = generate_solutions_whole_paths_v2(
                    n_particles, split_data, m, levitator, delta_time
                )
